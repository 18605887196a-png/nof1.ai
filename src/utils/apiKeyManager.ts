   ...
   try {
     ...
     if (response.ok) {
       ...
       if (data.choices && data.choices.length > 0 && data.choices[0].message) {
         logger.debug(`密钥健康检查成功: ${this.maskKey(apiKey)}, 响应时间: ${responseTime}ms`);
         return {
           isHealthy: true,
           responseTime
         };
       } else {
         logger.warn(`密钥健康检查失败 - 无效响应格式: ${this.maskKey(apiKey)}`);
         return {
           isHealthy: false,
           responseTime,
           error: "API返回无效响应格式"
       ...
       logger.debug(`密钥健康检查HTTP错误: ${this.maskKey(apiKey)}, 状态: ${response.status}, 错误: ${errorMessage}`);
       return {
         isHealthy: false,
         responseTime,
         error: errorMessage
     ...
     logger.debug(`密钥健康检查异常: ${this.maskKey(apiKey)}, 错误: ${errorMessage}, 响应时间: ${responseTime}ms`);
...
export function initializeApiKeyManager(
 apiKeys?: string[], // 可选参数，如果不提供则从环境变量读取
 ...
 if (apiKeys && apiKeys.length > 0) {
   ...
   if (!envKey) {
     throw new Error("未找到 OPENAI_API_KEY 环境变量，且未提供 apiKeys 参数");
   }
  
   // 解析逗号分隔的密钥
   if (envKey.includes(',')) {
     keysToUse = envKey.split(',').map(key => key.trim()).filter(key => key.length > 0);
   } else {
     // 单个密钥
     keysToUse = [envKey.trim()];
   }
 }


 if (keysToUse.length === 0) {
   throw new Error("没有找到可用的API密钥");
 }


 logger.info(`初始化API密钥管理器，使用 ${keysToUse.length} 个密钥`);
 globalApiKeyManager = new ApiKeyManager(keysToUse, baseURL, modelName);
