 ...
 * @param exchange 交易所（默认：'Gate'）
 * @param dragOffset 拖拽图表的偏移量（向左拖拽的像素数，默认：375）
...
// 添加额外的点击操作 - 无论什么时间周期都要执行
        ...
        try {
            // 新增操作：在 iframe 中先点击 canvas，再点击目标元素
            logger.info('步骤0: 在 iframe 中操作 canvas 和目标元素');
            ...
            try {
                ...
                logger.info(`当前页面有 ${frames.length} 个 frame`);

                // 尝试在所有 iframe 中查找目标元素
                let targetFrame = null;
                for (const frame of frames) {
                    try {
                        const canvasXPath = '/html/body/div[2]/div[3]/div/div[1]/div[2]/table/tr[13]/td[2]/div/canvas[2]';
                        const canvasElement = await frame.$(`xpath=${canvasXPath}`);
                        if (canvasElement) {
                            targetFrame = frame;
                            logger.info('找到包含目标 canvas 的 iframe');
                            break;
                        }
                    } catch (e) {
                        // 继续查找下一个 frame
                ...
                if (targetFrame) {
                    // 步骤1: 点击 canvas 元素
                    const canvasXPath = '/html/body/div[2]/div[3]/div/div[1]/div[2]/table/tr[13]/td[2]/div/canvas[2]';
                    logger.info(`在 iframe 中点击 canvas: ${canvasXPath}`);

                    await targetFrame.waitForSelector(`xpath=${canvasXPath}`, {timeout: 3000});

                    // 获取 canvas 的中心位置并点击
                    const canvasElement = await targetFrame.$(`xpath=${canvasXPath}`);
                    if (canvasElement) {
                        const box = await canvasElement.boundingBox();
                        if (box) {
                            const centerX = box.x + box.width / 2;
                            const centerY = box.y + box.height / 2;
                            logger.info(`点击 canvas 中心位置: (${centerX.toFixed(1)}, ${centerY.toFixed(1)})`);
                            await page.mouse.click(centerX, centerY);
                            logger.info('成功点击 canvas 元素');
                        }
                    }

                    // 优化：减少等待时间从1.5秒到800毫秒
                    ...
                    await targetFrame.click(`xpath=${targetXPath}`);
                    logger.info('成功点击目标元素');

                    // 优化：减少等待时间从1.5秒到500毫秒
                    await page.waitForTimeout(500);

                    // === 第二个 canvas 操作开始 ===
                    logger.info('步骤0.2: 操作第二个 canvas (tr[9])');

                    try {
                        // 步骤1: 点击第二个 canvas 元素 (tr[9])
                        const canvas2XPath = '/html/body/div[2]/div[3]/div/div[1]/div[2]/table/tr[9]/td[2]/div/canvas[2]';
                        logger.info(`在 iframe 中点击第二个 canvas: ${canvas2XPath}`);

                        await targetFrame.waitForSelector(`xpath=${canvas2XPath}`, {timeout: 3000});

                        // 获取第二个 canvas 的中心位置并点击
                        const canvas2Element = await targetFrame.$(`xpath=${canvas2XPath}`);
                        if (canvas2Element) {
                            const box2 = await canvas2Element.boundingBox();
                            if (box2) {
                                const centerX2 = box2.x + box2.width / 2;
                                const centerY2 = box2.y + box2.height / 2;
                                logger.info(`点击第二个 canvas 中心位置: (${centerX2.toFixed(1)}, ${centerY2.toFixed(1)})`);
                                await page.mouse.click(centerX2, centerY2);
                                logger.info('成功点击第二个 canvas 元素');
                            }
                        }
                        ...
                        // 步骤2: 点击第二个 canvas 显示出来的目标元素
                        const target2XPath = '/html/body/div[2]/div[3]/div/div[1]/div[2]/table/tr[9]/td[2]/div/div[2]/div[1]/div[3]';
                        logger.info(`在 iframe 中点击第二个目标元素: ${target2XPath}`);

                        await targetFrame.waitForSelector(`xpath=${target2XPath}`, {timeout: 3000});
                        await targetFrame.click(`xpath=${target2XPath}`);
                        logger.info('成功点击第二个目标元素');

                        // 等待操作完成
                        await page.waitForTimeout(500);

                    } catch (e2) {
                        logger.warn(`第二个 canvas 操作失败: ${e2}, 继续后续流程...`);
                    }
                    // === 第二个 canvas 操作结束 ===

                } else {
                    logger.warn('未找到包含目标元素的 iframe，跳过此步骤');
                }

            } catch (e) {
                logger.warn(`iframe 内操作失败: ${e}, 继续后续流程...`);
            }
            ...
            await page.waitForSelector(`xpath=${button9XPath}`, {timeout: 3000});
            ...
            await page.waitForSelector(`xpath=${menuItem1XPath}`, {timeout: 3000});
            ...
            await page.waitForSelector(`xpath=${finalButtonXPath}`, {timeout: 3000});
...
// 如果是iframe图表，额外等待确保iframe内容加载
        ...
        if (chartValidation.canvasCount > 0 && !chartValidation.hasContent) {
            logger.warn('图表元素存在但可能还在渲染，额外等待5秒...');
            await page.waitForTimeout(5000);
        }
        ...
        try {
            ...
            if (!targetElement) {
                ...
                if (viewport) {
                    ...
                    await page.mouse.move(centerX - dragOffset, centerY, {steps: 15}); // 使用传入的偏移量参数
                    await page.mouse.up();
                ...
                if (box) {
                    ...
                    // 拖拽动作：向左移动，使用传入的偏移量参数
                    await page.mouse.move(startX - dragOffset, startY, {steps: 15});
                    await page.mouse.up();

                    logger.info('图表拖拽完成');
                    await page.waitForTimeout(1000); // 优化：减少等待时间从1.5秒到1秒
                } else {
...
// === 裁剪图片：只保留左侧部分 ===
        ...
        logger.info('开始裁剪图片，去除右侧行情列表...');


        let croppedBuffer: Buffer;
        try {
            // 获取原始图片信息
            const metadata = await sharp(fullScreenshotBuffer).metadata();
            const originalWidth = metadata.width || 1920;
            const originalHeight = metadata.height || 1080;

            logger.info(`原始图片尺寸: ${originalWidth}x${originalHeight}`);

            const cropWidth = Math.floor(originalWidth * 0.8);
            const cropHeight = originalHeight;

            // 裁剪图片
            croppedBuffer = await sharp(fullScreenshotBuffer)
                .extract({
                    left: 0,
                    top: 0,
                    width: cropWidth,
            ...
            logger.warn(`图片裁剪失败: ${cropError}, 使用原始截图`);
            croppedBuffer = fullScreenshotBuffer;
        }


        timings['裁剪图片'] = Date.now() - tCrop;
// === 裁剪结束 ===
...
// 如果需要保存到本地文件
        if (saveScreenshotLocal && filepath) {
            try {
                ...
                logger.info(`裁剪后的截图已保存: ${filepath}`);
            } catch (saveError) {
                logger.warn(`保存文件失败: ${saveError}`);
            }
        }
