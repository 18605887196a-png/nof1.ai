...
// 在视觉模式下启用轻量采集，减少不必要的数据拉取
const strategy = process.env.TRADING_STRATEGY || 'visual-pattern';
const useLightCollection = strategy === 'visual-pattern';
...
for (const symbol of SYMBOLS) {
  try {
    ...
    // 视觉模式下走轻量数据路径：仅保留必要字段，其他细节由工具实时查询
    if (useLightCollection) {
      // 获取资金费率（轻量）
      let fundingRate = 0;
      try {
      ...
      // 构造轻量级 marketData，仅包含视觉提示所需的参考信息
      marketData[symbol] = {
      ...
      logger.debug(`${symbol} 使用轻量采集模式（visual-pattern），已跳过多时间框架与技术指标计算`);
      continue; // 直接进入下一个 symbol
    }
    ...
    const strategyInner = process.env.TRADING_STRATEGY || 'visual-pattern';
    // 视觉模式在轻量采集分支已 continue，不会进入此处
    const primaryTimeframe = '5m'; // 默认选择 5m（可按其他策略扩展）
    ...
    logger.debug(`${symbol} 使用${primaryTimeframe}周期数据作为主要指标，策略：${strategyInner}`);
...
// 0. 初始化API密钥管理器（支持多个密钥轮询）
try {
  const { initializeApiKeyManager } = await import("../utils/apiKeyManager");
  initializeApiKeyManager(); // 不传入参数，从环境变量读取
  logger.info("✅ API密钥管理器初始化成功");
} catch (error: any) {
  logger.error("API密钥管理器初始化失败:", error);
  throw new Error(`API密钥管理器初始化失败: ${error.message}`);
}
