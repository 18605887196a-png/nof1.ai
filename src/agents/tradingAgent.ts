...
你当前的角色和详细工作流程，已经在系统指令（Agent 的 instructions）中定义完备，这里不再重复。请严格遵循系统指令中的要求：优先调用 patternAnalysisTool 获取 Coinglass 视觉分析结论，再结合其他工具进行验证和执行，禁止凭空假设市场数据。
...
在后续分析中，如需具体的价格、技术指标或资金费率，请通过相应工具获取（例如：getMarketPriceTool、getTechnicalIndicatorsTool、getFundingRateTool），而不是直接依赖本提示中的静态信息。
...
当前无持仓，本轮可以更侧重新机会的筛选和布局，但仍需严格控制风险与仓位，避免一次性大额建仓。
...
当你基于 patternAnalysisTool 与上述验证工具，识别到 A 级信号（评分 ≥7 或明显强信号）且风险收益比合理时，应按照系统指令中的流程先验证、再计算仓位，最后使用 openPositionTool 执行，而不是只做文字分析。
...
2）持仓管理建议：
- 对每个当前持仓，说明你建议：继续持有 / 调整止损止盈 / 分批减仓 / 全部平仓，并给出 1–2 条最关键理由（至少包含一条与资金结构或走势结构相关）。


3）新机会评估：
- 如你认为某些交易对存在 A/B 级别的可执行机会，请指出标的、方向（多/空）、大致结构逻辑（例如：下跌后技术性反弹、下跌中继、底部构筑等），以及你是否计划在本轮实际执行（调用 openPositionTool）还是只列为后续观察对象。
...
请先根据系统指令合理调用工具，再基于工具返回结果给出你的文字决策。不要仅凭本提示中的静态数据直接下结论。`;
