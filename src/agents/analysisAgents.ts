...
#### 1）价格容忍带（动态 ±0.15–0.4%，默认 ±0.25%）


视觉分析工具（\`patternAnalysisMultiTool\`）在返回建议入场区间时，**应尽可能基于近期波动率（如 ATR 或价格标准差）动态标注容忍带**，格式示例如下：


> “建议做空参考区：84,500–84,600（容忍带 ±0.25%，基于15m波动率）”


若视觉工具**明确提供了容忍带百分比**，你必须优先采用该值进行判断。


若视觉工具**未明确提供容忍带**（仅给出价格区间），则默认使用 **±0.25%** 作为通用容忍阈值，并根据以下原则微调：


- **高波动场景**（如：小市值币种、重大事件期间、15m ATR > 1.0%）：
 - 可适度放宽至 **±0.3–0.4%**；
- **低波动场景**（如：主流币亚洲盘、15m ATR < 0.3%）：
 - 应收紧至 **±0.15–0.2%**。


> 💡 判断波动率高低的方法：调用 \`getTechnicalIndicatorsTool(symbol, '15m')\` 获取近期 ATR 或价格波动标准差。若工具返回 ATR14 = 250 点，当前价 = 85,000，则 ATR% ≈ 0.29%，属中等波动，采用默认 ±0.25% 即可。


当当前价格 P 与建议区间 [L, H] 的**相对偏离 ≤ 所采用的容忍带**，且同时满足：


- 未触发“贴支撑/贴阻力”禁止新仓规则；
- 未处于 RSI 极端（做空时 RSI14 ≥ 20；做多时 RSI14 ≤ 80）；


则视为“仍在合理入场区域内”，**允许轻仓尝试入场**，而不是因微小价差否定整个信号。
